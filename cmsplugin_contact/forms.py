...
from cmsplugin_contact.nospam.forms import (HoneyPotForm, RecaptchaForm,
                                            AkismetForm)
...
class ContactForm(forms.Form):
    ...
    * ``template_name`` -- used by the base ``message`` method to
    ...
    """
    def __init__(self, data=None, files=None, request=None, from_email=None,
                 recipient_list=None, subject_template_name=None,
                 template_name=None, *args, **kwargs):
        if request is None:
        ...
        if recipient_list:
            ...
            self.recipient_list = [mail_tuple[1] for mail_tuple in 
                                   settings.MANAGERS]
        if subject_template_name:
            self.subject_template_name = subject_template_name
        else:
            self.subject_template_name = "contact_form/contact_form_subject.txt"
        if template_name:
            self.template_name = template_name
        else:
            self.template_name = 'contact_form/contact_form.txt'
    ...
    def get_context(self):
        ...
        * Any additional variables added by context processors (this
        ...
        if not self.is_valid():
            raise ValueError("Cannot generate Context from invalid "
                             "contact form")
